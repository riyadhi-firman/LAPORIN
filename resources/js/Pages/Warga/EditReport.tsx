...
export default function EditReport({ report, categories }: PageProps<{ report: Report & { category_id?: number, latitude?: number, longitude?: number }, categories: { id: number, name: string }[] }>) {
    // Note: To send files via PUT/PATCH in Laravel from FormData, we must use POST method and add _method='PUT'
    const { data, setData, post, processing, errors } = useForm({
        ...
        latitude: report.latitude || null as number | null,
        longitude: report.longitude || null as number | null,
    });

    const [preview, setPreview] = useState<string | null>(report.image ? `/storage/${report.image}` : null);
    ...
    return (
        <AuthenticatedLayout header={`Edit Laporan #${report.id}`}>
            <Head title="Edit Laporan" />

            <div className="mb-6">
                <Button variant="ghost" asChild className="pl-0 hover:bg-transparent">
                    <Link href={route('reports.show', report.id)} className="text-muted-foreground hover:text-foreground">
            ...
            <Card className="max-w-2xl mx-auto">
                <CardHeader>
                    <CardTitle>Perbarui Laporan</CardTitle>
                    <CardDescription>
                        Ubah detail laporan Anda. Pastikan informasi yang diberikan akurat.
                    </CardDescription>
                </CardHeader>
                <form onSubmit={submit} encType="multipart/form-data">
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground mb-2">Klik pada peta untuk memperbarui lokasi spesifik.</p>
                            <MapPicker 
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="image">Ubah Foto/Bukti Laporan (Biarkan kosong jika tidak diubah)</Label>
